...
categaoryDetails: Category  = {} as Category;
...
ngOnInit(): void {
 ...
 this._ecomdataService.getCategoryDetails(this.categoryId).subscribe({
  next:(response)=>{
   this.categaoryDetails = response.data
  },
 ...
 this._ecomdataService.getAllProducts().subscribe({
  next:(response)=>{
    ...
    let newProducts = this.products.filter(product=> product.category._id == this.categoryId)
